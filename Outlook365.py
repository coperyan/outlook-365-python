...
class OutlookMessage(Outlook):
    ...
    def __init__(
        ...
        super(OutlookMessage,self).__init__(
            username,
            password,
            username if not email else email
        )
        ...
        self.importance = importance if importance in ['High','Low','Normal'] else 'Normal'
        self.msg_item = None
        self.build_message()
    ...
    def build_message(self):
        ...
        """
        if len(self.to_recipients) > 0 or len(self.cc_recipients) > 0 or len(self.bcc_recipients) > 0:
...
class OutlookMailboxItem:
    ...
    def get_metadata(self):
        ...
        self.to_recipients = [] if not self.msg.to_recipients else [
            x.email_address for x in self.msg.to_recipients
        ]
        self.cc_recipients = [] if not self.msg.cc_recipients else [
            x.email_address for x in self.msg.cc_recipients
        ]
        try:
    ...
    def save_attachments(self,path,contains=None):
        ...
        for attachment in attachments:
            ...
            if os.path.exists(save_path):
                save_path = os.path.join(path,name.replace('.xlsx',f'_{(len(os.listdir(path))+1)}.xlsx'))
...
class OutlookMailboxSearch(Outlook):
    ...
    def __init__(
        ...
        self.folder = self.account.inbox if mailbox == 'inbox' \
            else self.account.sent if mailbox == 'sent' \
